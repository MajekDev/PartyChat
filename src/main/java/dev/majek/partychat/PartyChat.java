package dev.majek.partychat;

import dev.majek.partychat.command.*;
import dev.majek.partychat.data.ConfigUpdater;
import dev.majek.partychat.data.DataManager;
import dev.majek.partychat.data.Metrics;
...
import dev.majek.partychat.sqlite.Database;
...
public final class PartyChat extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        Bukkit.getConsoleSender().sendMessage(Chat.colorize("   &b(__)   &e\\___)     &8Last updated &912/29/2020 &8by &bMajekdor"));
        ...
        try {
            ConfigUpdater.update(instance, "messages.yml", messagesFile, Collections.emptyList());
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        if (this.getConfig().getBoolean("persistent-parties")) {
            ...
            db.getParties();
            db.clearTable();
            Bukkit.getConsoleSender().sendMessage("[PartyChat] Loading saved parties from database...");
        }
    ...
    public void onDisable() {
        ...
        if (this.getConfig().getBoolean("persistent-parties")) {
            ...
            Bukkit.getConsoleSender().sendMessage("[PartyChat] Saving active parties to database...");
        }
